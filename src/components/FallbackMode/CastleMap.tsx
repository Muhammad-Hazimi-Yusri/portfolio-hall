...
const ZONES: ZoneDef[] = [
  { id: 'main-hall',  x: -8,  y: -22, w: 16, h: 14, label: 'Main Hall',  labelX: 0,  labelY: -16.5 },
  { id: 'courtyard',  x: -8,  y: -8,  w: 16, h: 16, label: 'Courtyard',  labelX: 0,  labelY: -0.5  },
  { id: 'reception',  x: -5,  y:  8,  w: 10, h: 10, label: 'Reception',  labelX: 0,  labelY: 12   },
  { id: 'garden',     x:  8,  y: -6,  w: 12, h: 12, label: 'Garden',     labelX: 14, labelY: -0.5  },
]
...
export function CastleMap({ activeZone, pois, onZoneClick, onPOIClick }: Props) {
  return (
    ...
    >
      ...
      {/* ── Parchment backdrop ── */}
      <rect
        x="-10" y="-24" width="22" height="46" rx="1"
      ...
      <rect
        x="-9.5" y="-23.5" width="21" height="45" rx="0.5"
        fill="none"
        stroke={GOLD}
        strokeWidth="0.15"
        strokeOpacity="0.3"
      ...
      <g pointerEvents="none">
        ...
        {/* Main Hall — side paintings */}
        {[[-8, -19], [-8, -15]].map(([px, py], i) => (
      ...
      {/* ── Zone labels ── */}
      <g pointerEvents="none">
        {ZONES.map(({ id, label, labelX, labelY }) => {
          const isActive = activeZone === id
          return (
            <text
              key={id}
